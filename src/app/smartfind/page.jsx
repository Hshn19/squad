...
const TAB_CARD_BG    = { travel: 'rgba(106,61,232,0.08)', food: '#fff0e8', shop: '#e8f5e9' };
const TAB_ICON_COLOR = { travel: '#6a3de8', food: '#FF6B35', shop: '#2E7D32' };
const TAB_FILTERS    = {
  travel: ['All', 'Hotels', 'Flights', 'Activities'],
  food:   ['All', 'Dine-in', 'Delivery', 'Halal'],
  shop:   ['All', 'Gifts', 'Gadgets', 'Groceries'],
};
const EMOJI_MAP  = { travel: '✈️', food: '🍽️', shop: '🛍️' };
const COLOR_MAP  = { travel: '#6a3de8', food: '#FF6B35', shop: '#00C896' };

function tagStyle(t) {
  if (/off|promo|deal|free|discount|budget/i.test(t)) return { bg: '#FAEEDA', color: '#854F0B' };
  if (/premium|luxury|5-star/i.test(t))               return { bg: 'rgba(106,61,232,0.1)', color: '#9c6fff' };
  return { bg: '#E1F5EE', color: '#0F6E56' };
}

export default function SmartFindPage() {
  const { showToast, bookDeal } = useApp();
  const router = useRouter();

  const [tab,          setTab]          = useState('travel');
  const [contrib,      setContrib]      = useState(75);
  const [activeFilter, setActiveFilter] = useState('All');
  const [booked,       setBooked]       = useState(null); // title of just-booked item
  ...
  function handleBook(item, perHead) {
    bookDeal(item.title, item.price, perHead, MEMBERS, EMOJI_MAP[tab], COLOR_MAP[tab]);
    setBooked(item.title);
    setTimeout(() => {
      setBooked(null);
      router.push('/squad');
    }, 1800);
      ...
      <div style={{ background: 'linear-gradient(160deg, #1a0a3d, #3d1f8a)', padding: '48px 20px 24px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', marginLeft: 4 }}>{MEMBERS.length} members</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', background: '#fff', borderBottom: '0.5px solid #eee', padding: '0 16px' }}>
        {Object.keys(SF_DATA).map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            color: tab === t ? '#6a3de8' : '#bbb',
            borderTop: 'none', borderLeft: 'none', borderRight: 'none',
            borderBottom: tab === t ? '2px solid #6a3de8' : '2px solid transparent',
      ...
      <div style={{ padding: '12px 12px 0' }}>
        ...
        {booked && (
          <div style={{
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>Booked! Added to Squad goals</p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.8)' }}>"{booked}" — navigating to Squad…</p>
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.8px', color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', marginBottom: 3 }}>AI Coach</p>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10, overflowX: 'auto', paddingBottom: 2 }}>
          {TAB_FILTERS[tab].map((f) => (
            <button key={f} onClick={() => setActiveFilter(f)} style={{
              ...
              border: `0.5px solid ${activeFilter === f ? '#6a3de8' : '#ddd'}`,
              background: activeFilter === f ? '#6a3de8' : '#fff',
        ...
        {data.items.map((item, idx) => {
          ...
          return (
            <div key={idx} style={{
              ...
              marginBottom: 12, overflow: 'hidden', opacity: over ? 0.5 : 1,
            }}>
              {/* Card image block */}
              <div style={{
                height: 88, background: TAB_CARD_BG[tab],
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
              ...
              <div style={{ padding: '10px 12px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#00C896', marginTop: 1 }}>RM {perHead} p/person</div>
                  </div>
                </div>
                ...
                {/* Vote strip */}
                <div style={{
                  background: 'rgba(106,61,232,0.08)', borderRadius: 8, padding: '6px 10px',
                ...
                <div style={{
                  ...
                  paddingTop: 8, borderTop: '0.5px solid #f2f2f2',
                }}>
                  {/* Partner badge */}
                  ...
                  {over ? (
                    <button disabled style={{
                      ...
                      fontSize: 11, fontWeight: 600, background: '#f0f0f0',
                      color: '#bbb', cursor: 'not-allowed', fontFamily: 'inherit',
                    }}>Can't afford</button>
                  ) : (
                    <button
                      onClick={() => handleBook(item, perHead)}
                      style={{
                        border: 'none', borderRadius: 8, padding: '7px 14px',
