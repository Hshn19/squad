...
const GOAL_COLORS = ['#6a3de8', '#00C896', '#F8B500', '#FF6B6B', '#4ECDC4', '#9C8FFF'];
const GOAL_EMOJIS = ['🎯', '✈️', '💻', '🏠', '🎓', '🎮', '🍕', '💍', '🚗', '🌴'];
...
export default function SquadPage() {
  ...
  const [newColor,  setNewColor]  = useState('#6a3de8');
  ...
  const handleCreateGoal = () => {
    ...
    setNewName(''); setNewTarget(''); setNewEmoji('🎯'); setNewColor('#6a3de8');
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, #1a0a3d, #3d1f8a)', padding: '48px 20px 56px' }}>
        ...
        <div style={{
          ...
          <div>
            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', marginBottom: 2 }}>Total across all goals</p>
            <p style={{ fontSize: 22, fontWeight: 700, color: '#fff' }}>RM {totalSaved.toLocaleString()}</p>
          </div>
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', marginBottom: 2 }}>Active goals</p>
      ...
      <div style={{ marginTop: -28 }}>
        {squadGoals.length === 0 && (
          <div style={{
            ...
            padding: 32, textAlign: 'center', border: '0.5px solid #eef0f4',
          }}>
            <p style={{ fontSize: 28, marginBottom: 8 }}>🎯</p>
            <p style={{ fontSize: 14, fontWeight: 600, color: '#1a1a2e', marginBottom: 4 }}>No goals yet</p>
            <p style={{ fontSize: 12, color: '#aaa', marginBottom: 16 }}>Create a goal or book a deal from Smart Find</p>
            <button
              onClick={() => setShowCreateModal(true)}
              style={{
          ...
          return (
            <div key={goal.id} style={{
              ...
              <div style={{ padding: 14 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                          ...
                          <span style={{ fontSize: 11, color: '#aaa' }}>
                            RM {member.contributed} / RM {member.target}
                          </span>
                        </div>
                        <div style={{ height: 4, background: '#f0f0f0', borderRadius: 2 }}>
                          <div style={{
                            height: '100%', borderRadius: 2,
                            background: member.color, width: `${mp}%`,
                            transition: 'width .4s',
                          }} />
                        </div>
                      </div>
                      {member.contributed >= member.target && (
                        <span style={{ fontSize: 14, color: '#00C896' }}>✓</span>
                      )}
                    </div>
                  );
                })}
                ...
                {progress >= 100 && (
                  <div style={{
                    background: 'rgba(0,200,150,0.1)', borderRadius: 10,
                    padding: '8px 12px', marginTop: 4,
                    display: 'flex', alignItems: 'center', gap: 6,
                    border: '0.5px solid rgba(0,200,150,0.3)',
                  }}>
                    <span style={{ fontSize: 16 }}>🎉</span>
                    <p style={{ fontSize: 12, fontWeight: 600, color: '#00C896' }}>Goal reached! Time to celebrate.</p>
                  </div>
                )}
              </div>
        ...
        <div
          ...
          style={{
            margin: '0 14px 10px', background: '#fff', borderRadius: 18,
            border: '2px dashed rgba(106,61,232,0.25)', padding: 20,
        ...
        >
          ...
          <p style={{ fontSize: 11, color: '#aaa' }}>Or book a deal from Smart Find</p>
        </div>
      </div>
      ...
      {activeGoal && (
        <div
          ...
          style={{
            position: 'fixed', inset: 0, background: 'rgba(26,10,61,0.75)',
            ...
            {contributed ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 0' }}>
                ...
                <p style={{ fontSize: 13, color: '#aaa', marginTop: 6 }}>
                  RM {parseFloat(contributeAmount).toFixed(2)} added to {activeGoal.name}
                </p>
              </div>
            ) : (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 12, color: '#aaa' }}>
                      RM {activeGoal.current.toLocaleString()} of RM {activeGoal.target.toLocaleString()} saved
                    </p>
                  </div>
                  <button
                    onClick={() => { setActiveGoal(null); setContributeAmount(''); }}
                    style={{ background: 'none', border: 'none', fontSize: 20, color: '#bbb', cursor: 'pointer' }}
                  >✕</button>
                </div>
                ...
                <div style={{ position: 'relative', marginBottom: 12 }}>
                  <span style={{
                    ...
                    transform: 'translateY(-50%)', color: '#aaa', fontWeight: 600, fontSize: 14,
                  }}>RM</span>
                  <input
                    type="number"
                    ...
                    style={{
                      ...
                      border: '0.5px solid #e0e0e8', fontSize: 22, fontWeight: 700,
                  ...
                  {[20, 50, 100, 200].map((amt) => (
                    <button
                      ...
                      style={{
                        ...
                        border: `0.5px solid ${contributeAmount === amt.toString() ? '#6a3de8' : '#e0e0e8'}`,
                        background: contributeAmount === amt.toString() ? '#6a3de8' : '#fff',
                ...
                <button
                  ...
                  style={{
                    ...
                    background: contributeAmount && parseFloat(contributeAmount) > 0
                      ? 'linear-gradient(135deg, #6a3de8, #3d1f8a)' : '#e0e0e8',
      ...
      {showCreateModal && (
        ...
        >
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
              {GOAL_EMOJIS.map((e) => (
                <button key={e} onClick={() => setNewEmoji(e)} style={{
                  ...
                  border: `2px solid ${newEmoji === e ? '#6a3de8' : '#e0e0e8'}`,
                  background: newEmoji === e ? 'rgba(106,61,232,0.1)' : '#fff',
                  cursor: 'pointer',
                }}>{e}</button>
              ))}
            </div>
            ...
            {newName && (
              <div style={{
                background: newColor + '12', borderRadius: 14, padding: '12px 14px',
                ...
                <div>
                  ...
                  {newTarget && (
                    <p style={{ fontSize: 12, color: '#aaa' }}>Target: RM {parseFloat(newTarget).toLocaleString()}</p>
                  )}
                </div>
              </div>
            )}
